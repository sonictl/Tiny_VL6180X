/***
 * @file Tiny_VL6180X.cpp
 *       modified from Adafruit_VL6180X.cpp for adapting Attiny85 MCU.
 *       replayced the "Adafruit" with "Tiny"
 * 
 */
...
Tiny_VL6180X::Tiny_VL6180X(uint8_t i2caddr) : _i2caddr(i2caddr) {}
...
boolean Tiny_VL6180X::begin(TwoWire *theWire) {
...
//   i2c_dev = new Adafruit_I2CDevice(_i2caddr, _i2c);
  i2c_dev = new Tiny_I2CDevice(_i2caddr, _i2c);
  if (!i2c_dev->begin())
    return false;
...
boolean Tiny_VL6180X::setAddress(uint8_t newAddr) {
...
uint8_t Tiny_VL6180X::getAddress(void) { return _i2caddr; }
...
void Tiny_VL6180X::loadSettings(void) {
  // load settings!
...
uint8_t Tiny_VL6180X::readRange(void) {
...
boolean Tiny_VL6180X::startRange(void) {
  // wait for device to be ready for range measurement
  while (!(read8(VL6180X_REG_RESULT_RANGE_STATUS) & 0x01))
    ;
...
boolean Tiny_VL6180X::isRangeComplete(void) {
...
boolean Tiny_VL6180X::waitRangeComplete(void) {
...
uint8_t Tiny_VL6180X::readRangeResult(void) {
...
void Tiny_VL6180X::startRangeContinuous(uint16_t period_ms) {
...
void Tiny_VL6180X::stopRangeContinuous(void) {
...
uint8_t Tiny_VL6180X::readRangeStatus(void) {
...
float Tiny_VL6180X::readLux(uint8_t gain) {
  uint8_t reg;
...
void Tiny_VL6180X::setOffset(uint8_t offset) {
  // write the offset
...
void Tiny_VL6180X::getID(uint8_t *id_ptr) {
...
uint8_t Tiny_VL6180X::read8(uint16_t address) {
  uint8_t buffer[2];
...
uint16_t Tiny_VL6180X::read16(uint16_t address) {
  uint8_t buffer[2];
  buffer[0] = uint8_t(address >> 8);
  buffer[1] = uint8_t(address & 0xFF);
  i2c_dev->write(buffer, 2);
...
// write 1 byte
void Tiny_VL6180X::write8(uint16_t address, uint8_t data) {
  uint8_t buffer[3];
  buffer[0] = uint8_t(address >> 8);
  buffer[1] = uint8_t(address & 0xFF);
  buffer[2] = data;
...
// write 2 bytes
void Tiny_VL6180X::write16(uint16_t address, uint16_t data) {
  uint8_t buffer[4];
  buffer[0] = uint8_t(address >> 8);
  buffer[1] = uint8_t(address & 0xFF);
